...
- (void)updateFrames
{
    ...
    self.indexLabel.xl_centerY = iPhoneX ? 51 : 35;
...
- (void)delete
{
    if (self.deleteWithTag) {
        self.deleteWithTag(self.currentImageIndex);
    }
    ...
    NSMutableArray *imageArr = [[NSMutableArray alloc] initWithArray:self.images];
    [imageArr removeObjectAtIndex:self.currentImageIndex];
    self.images = imageArr;
    if (self.delecteImageIndex) {
        self.delecteImageIndex(self.currentImageIndex);
    }
...
- (void)saveImage
{
    ...
    if (status == PHAuthorizationStatusRestricted ||
        ...
        [SVProgressHUD showInfoWithStatus:@"相册使用权限被关闭，无法保存照片"];
    }else{
...
- (void)updateIndexContent
{
    UIPageControl *pageControl = (UIPageControl *)self.pageControl;
    pageControl.currentPage = self.currentImageIndex;
    NSString *title = [NSString stringWithFormat:@"%ld / %ld",self.currentImageIndex+1,self.imageCount];
 ...
 */
+ (instancetype)showPhotoBrowserWithImages:(NSArray *)images currentImageIndex:(NSInteger)currentImageIndex isShowDelete:(BOOL)isShowDelete
{
    ...
    browser.actionSheetCancelTitle = @"取消";
    browser.actionOtherButtonTitles = @[@"保存"];
    if (isShowDelete) {
        browser.actionSheetDeleteButtonTitle = @"删除";
    }
    [browser show];
    return browser;
}
